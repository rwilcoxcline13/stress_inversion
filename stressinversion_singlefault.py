...
sangle = np.array([0.3, 0.5, 0.25])
fangle = np.array([np.pi/3, np.pi/6, 0])
rakeobs = -28.067

n_steps = 10**6
kappa = 100000
mu_initial = np.array([0, 0, 0, 1])
calculated_rakes = np.zeros((n_steps, 1))
residual = np.zeros((n_steps, 1))
likelihood = np.zeros((n_steps, 1))


steps = np.zeros((n_steps, len(mu_initial)))
model_parameters = np.zeros((n_steps, len(mu_initial)))
...
for i in range(n_steps):
    ...
    if i == 0:
        ...
        model_parameters[i] = cartesian2spherical(initial_step)

        rake_estimated = Rake = Resolve_ShearStressDir(pstress, model_parameters[i], fangle)

        residual[i] = rakeobs - rake_estimated

        likelihood[i] = np.exp(np.cos(np.abs(residual[i]*np.pi/180)))
        ...
        possible_model_parameters = cartesian2spherical(step)



        rake_estimated = Rake = Resolve_ShearStressDir(pstress, possible_model_parameters.flatten(), fangle)

        possible_residual = rakeobs - rake_estimated

        likelihood_current_step = np.exp(np.cos(np.abs(possible_residual*np.pi/180)))
        ...
        if LR >= random_number:
                ...
                model_parameters[i] = possible_model_parameters

                calculated_rakes[i] = rake_estimated

                residual[i] = possible_residual
...
plt.scatter(count, residual, alpha = 0.2)
